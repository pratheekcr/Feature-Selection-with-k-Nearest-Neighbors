...
def forwardSelection(data,features):
	...
	total=end-start
	print "Finished search!! The best feature subset is {",','.join(str(i) for i in bestFeatureSet),"},with Accuracy",globalAccuracy,"%\n"
...
def backwardElimination(data,features):
	...
	print "Running nearest neighbor with all ",features," features, using \"leaving-one-out\" evaluation, the accuracy is",accuracy,"%\n"
	print "Beginning search.\n"
	start = time.time()
	posFlags = [i for i in range(1,features+1)]
